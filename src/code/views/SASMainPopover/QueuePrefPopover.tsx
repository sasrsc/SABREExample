...
export class QueuePrefPopover extends React.Component<
...
> {
  ...
  handleChange(e): void {
    if (
      e.target.name === "actionCode" &&
      this.payloads[e.target.value] != null
    ) {
      this.setState({
        [e.target.name]: e.target.value,
        payload: new XmlTools().prettifyXml(this.payloads[e.target.value]),
      });
    } else {
    ...
    if (!_.isEmpty(this.state.response)) {
      if (!_.isEmpty(this.state.rsfilter) && this.state.shouldParse == true) {
        try {
          let res = xm.runXPath(
            this.state.rsfilter,
            xm.stringToXml(this.state.response)
          );
          var node = null;
    ...
    return (
      ...
      >
        <Input
          ...
          name="actionCode"
          title="Action Code"
          placeHolder="enter Sabre SOAP API Action Code"
        ...
        <Input
          ...
          name="payload"
          title="XML Request"
          placeHolder="enter Service Request payload (XML content)"
        ...
        <Payload
          ...
          name="plData"
          title="XML Response"
          value={this.getPrettyParsedXML()}
        />
        <InputGroup
          componentType="check"
          componentValue={this.state.shouldParse}
          groupPosition="prepend"
          name="rsfilter"
          title="Parse response"
          placeHolder="enter a XPATH expression"
          value={this.state.rsfilter}
          handleChange={this.handleChange}
