...
export class CstBusinessForm extends React.Component<
  CstBusinessFormProps,
  CstBusinessFormState
> {
  constructor(e) {
    super(e);
  ...
  handleExecute(): void {
    let pl = this.payloads.AddRemarkRQ.replace(
      ...
      this.payloads.Remark.replace(
        "{Text}",
        "ACK2".concat(this.state.travelType)
      )
    );
  ...
  renderButtons(): JSX.Element[] {
    return [
      ...
      <Button
        ...
        title="Execute"
        disabled={
          this.readAckStatus(this.props.pnrData, "ACK2") ||
    ...
    return (
      ...
      >
        <Payload
          type="json"
          name="plData"
          title="Business Reservation data"
          value={JSON.stringify(this.props.pnrData)}
        />
        <Checkbox
          name="ackStage"
          ...
          placeHolder=""
          disabled={this.readAckStatus(this.props.pnrData, "ACK2")}
        />
      </PopoverForm>
    );
  }
